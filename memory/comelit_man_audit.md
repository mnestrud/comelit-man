...
**Delta review 2026-08-28 (v1.5.0, not a full sweep):** rows below carry their original Verified dates and are formally STALE by the freshness rule (>90 days AND 1.0.x-era). Spot-verified today: CI green (ruff 0.15.15, mypy 2.1.0, hassfest, HACS); tests 981 passed / 9 skipped; **total coverage 96%** (down from the 100% recorded at 1.0.2 — remaining gaps: video_call.py inbound `start_inbound` body, coordinator inbound path, door.py, button.py; all new-in-1.3/1.4/1.5 code is covered). Platforms grew to BINARY_SENSOR/BUTTON/CAMERA/EVENT/IMAGE/LOCK/SENSOR. Tier claims below are believed to hold except Silver test-coverage, now PARTIAL (96%, was 100%); a full re-sweep is due.

**Bronze `brands` row is no longer an accepted-FAIL (2026-08-28).** Core 2026.3 lets custom integrations ship brand images locally, and `custom_components/comelit_man/brand/icon.png` is present — HACS brands validation passes. Adding `logo.png` plus `dark_`/`@2x` variants would complete the set. Ref: https://developers.home-assistant.io/blog/2026/02/24/brands-proxy-api/
...
**Version at audit:** 1.0.2 (full sweep); delta-reviewed at 1.5.0
